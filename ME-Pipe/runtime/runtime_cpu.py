...
def worker_func(*pargs, **kwargs): # per process
    from me_pipe_cpu import Worker
...
def run(args, real_dataset, create_model, create_optimizer, get_train_steps=None, get_lr_sched=None, compute_loss=None, save_model=None): # main process
    ...
    #     for vt in rTASKS[rank]:
    #         print(vt)
    # exit(0)

    # for rank in range(CONFIGS["N"]):
    #     for vt in rTASKS[rank]:
    #         # print(f"vt.type:{vt.type}")
    #         if vt.type != "UPD":
    #             for layer_id in vt.layers:
    #                 vlayer = model[layer_id][0]
    #                 if hasattr(vlayer, 'vt'):
    #                     exit(0)
    #                 vlayer.vt = vt
    ...
    # for rank in range(CONFIGS["N"]):
    #     # ✅
    #     layers_in_rank = [layer_id for vt in rTASKS[rank] if vt.type != "UPD" for layer_id in vt.layers]
    #     print(f"rank{rank} including: {layers_in_rank}")
    #     layers_in_all_rank.append(layers_in_rank)
    #     model_of_cur_rank = [model[layer_id] for layer_id in layers_in_rank] 
    #     models.append(model_of_cur_rank)

    #     # initialize empty model on CPU
    #     empty_model_of_cur_rank = [] 
    #     for vlayer, _, _ in model_of_cur_rank:
    #         with torch.no_grad():
    #             vlayer_copy = deepcopy(vlayer)
    #         delete_param_grad_buf(vlayer_copy)
    #         empty_model_of_cur_rank.append(vlayer_copy)
    #     empty_models.append(empty_model_of_cur_rank)
    ...
    from shared_optim_cpu import SharedOptimCPU_worker6_4_2
    ...
    for id, ((vlayer, _, _), optim) in enumerate(zip(shared_model, optimizer)): 
        shared_optimizer.append(SharedOptimCPU_worker6_4_2(vlayer, optim, id))
    pcm.print("shared optimizer created")

    # ====my version====
    # optimizers = []
    # for rank in range(CONFIGS["N"]):
    #     optim_of_cur_rank = [optimizer[layer_id] for layer_id in layers_in_all_rank[rank]]
    #     optimizers.append(optim_of_cur_rank)

    # shared_models = models
    # shared_optimizers = []
    # for shared_model, optim_of_cur_rank in zip(shared_models, optimizers):
    #     shared_optimizer = []
    #     for id, ((vlayer, _, _), optim) in enumerate(zip(shared_model, optim_of_cur_rank)): 
    #         shared_optimizer.append(SharedOptimCPU(vlayer, optim, id))
    #     shared_optimizers.append(shared_optimizer)
    # pcm.print("shared optimizer created")
    # ==================
    ...
    # for rank in range(CONFIGS["N"]):
    #     p = mp.Process(target=worker_func, 
    #                     args=(args, real_dataset, shared_models[rank], shared_optimizers[rank], empty_models[rank], get_lr_sched, compute_loss, save_model, prof['XMETA'], prof['TMETA'], rTASKS, CONFIGS, rank),
    #                     name="rank%d"%rank)
    #     # NOTE: this moves parameter from pinned memory to shared memory
    #     p.start()
    #     processes.append(p)
