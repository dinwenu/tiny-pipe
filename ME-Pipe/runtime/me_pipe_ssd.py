...
def _forward_pre_hook(module, input):
    ...
    if _is_first_ubatch and not _already_get[_current_layer._gl_layer_num]:
        ...
        assert layer_id == _current_layer._gl_layer_num, "layer_id != _current_layer._gl_layer_num"
        # waiting_info = _layer_waiting_futs(_current_layer, nvprof)
        # print(f"rank:{_handler.rank}, waiting_info: {waiting_info}", flush=True)
        # debug_info += waiting_info
    ...
    if _is_first_ubatch and _handler.should_stay_in_gpu[_current_layer._gl_layer_num]:
        _local_model[_current_layer._gl_layer_num].set_param_requiresgrad()
        _handler.shared_optimizer[_current_layer._gl_layer_num].make_shared_model_point_to_pinned(_local_model[_current_layer._gl_layer_num].get_pinned_data().pinned_layer)
    ...
    if _is_first_ubatch and not _requires_grad and _current_layer._gl_which_layer_to_cuda_pre_fwd_required:
        ...
        # print(f"rank:{_handler.rank}, --------->suc_vt:{prefetched_layer_vt}", flush=True)
        ...
        if _which_layer_to_cuda not in _layers_in_gpu:
            ...
            # if _which_layer_to_cuda not in _layers_in_rank:
            #     futs = [_handler.layer_to_cuda(_which_layer_to_cuda, _suc_vt)]
            #     _append_loading_futs(_which_layer_to_cuda, futs)

            #     _layers_in_rank.append(_which_layer_to_cuda)
...
def _forward_post_hook(module, input, output):
    ...
    debug_info = f"rank:{_handler.rank}, 正在执行第{_current_layer._gl_layer_num}层的FWD post hook \n"
    ...
    if _is_last_ubatch and not _requires_grad and _current_layer._gl_which_layer_to_cpu_post_fwd_required:
        ...
        if not _handler.should_stay_in_gpu[_which_layer_to_cpu]:
            ...
            if _which_layer_to_cpu in _special_cuda_queue.keys():
                ...
                _local_model[_which_layer_to_cpu].return_pinned_data()
            # futs = [_handler.layer_to_cpu_fwd_remote(_which_layer_to_cpu, _cache_unit)]
...
def _backward_post_hook(module, grad_in, grad_out):
    ...
    if _is_last_ubatch and _current_layer._gl_which_layer_to_cpu_post_bwd_required and not only_has_one_layer:
        ...
        print(f"rank:{_handler.rank},current layer is {module._gl_layer_num}")
        _current_layer = _layers[_current_layer._gl_layer_num+1]
        print(f"\tredirect to layer{_current_layer._gl_layer_num} in BWD post hook")
        ...
        # futs = [
        #     _handler.layer_to_cpu_bwd_remote(
        #         _which_layer_to_cpu, _vt
        #     )
        # ]
        # _update_handler.iput(_vt, _which_layer_to_cpu, futs)

        _update_handler.iput(_vt, _which_layer_to_cpu)

        # if _which_layer_to_cuda is not None:
        #     print(f"\tadd layer{_current_layer._gl_layer_num} fut to {_which_layer_to_cuda[0]}")
        #     _append_offloading_futs(_layers[_which_layer_to_cuda[0]], futs, fwd=False)
        # else:
        #     print(f"\tadd layer{_current_layer._gl_layer_num} fut to {_last_run_suc_vt_layer}")
        #     _append_offloading_futs(_layers[_last_run_suc_vt_layer], futs, fwd=False)

        print(f"\tadd layer{_current_layer._gl_layer_num} fut to itself")
...
class GL_PretrainHanlder:
    def __init__(
        ...
        # 
        # self.layer_id_to_prefetched_layer_vt = {}
        # for layer_idx, layer_id in enumerate(self.layers_id_in_rank):
        #     prefetch_idx = layer_idx + self.gl_window_size
        #     if prefetch_idx < len(self.layers_id_in_rank):
        #         vt = self.layer_idx_to_vt[prefetch_idx]
        #     else:
        #         vt = None
...
class Worker(object): # each rank process
    def __init__(self, args, real_dataset, shared_model, shared_optimizer, empty_model, get_lr_sched, compute_loss, save_model, XMETA, TMETA, rTASKS, CONFIGS, rank, transformer_layer_info=None, special_layers_info=None):
        self.args = args
        ...
        self.transformer_layer_info = transformer_layer_info
        self.special_layers_info = special_layers_info

        # for vt in rTASKS[self.rank]:
        #     print(f"rank:{self.rank}, vt.layers:{vt.layers}, vt.type:{vt.type}")
        # exit(0)

        # for layer_id, (layer,_,_) in enumerate(self.shared_model):
        #     for param in layer.parameters():
        #         assert param.grad is not None, f"layer:{layer_id}, model has grad = None when make_shared_model_point_to_pinned"
        ...
        # self.max_len = max(window_size, layer_num)
        self.max_len = min(window_size*2, layer_num)
        self.cuda_cache_queue = deque(maxlen=window_size*2)
        ...
        special_layers_param_idx_to_shape = {}
        for layer_id, layer_info in self.special_layers_info.items():
            special_layers_param_idx_to_shape[layer_id] = {}
            if layer_id in self.special_layers_in_rank:
                for param_idx, shape in layer_info['param_idx_to_shape'].items():
                    special_layers_param_idx_to_shape[layer_id][param_idx] = shape
            ...
            for cpu_m in _layer.modules():
                for key, param in cpu_m._parameters.items():
                    if param is not None:
                        unit[global_param_index] = torch.empty(special_layers_param_idx_to_shape[layer_id][global_param_index], device=self.rank)
                        unit[str(global_param_index)+".grad"] = torch.zeros_like(unit[global_param_index], device=self.rank)
        ...
        transformer_layer_param_idx_to_shape = self.transformer_layer_info['param_idx_to_shape']
        for i in range(window_size+2):
            ...
            for cpu_m in _layer.modules():
                for key, param in cpu_m._parameters.items():
                    if param is not None:
                        unit[global_param_index] = torch.empty(transformer_layer_param_idx_to_shape[global_param_index], device=self.rank)
                        unit[str(global_param_index)+".grad"] = torch.zeros_like(unit[global_param_index], device=self.rank)
                        global_param_index += 1
                for key, buf in cpu_m._buffers.items():
                    if buf is not None:
        ...
        #         for key, param in cpu_m._parameters.items():
        #             if param is not None:
        #                 unit[global_param_index] = torch.empty_like(param, device=self.rank, pin_memory=True)
        #                 global_param_index += 1
        #         for key, buf in cpu_m._buffers.items():
        #             if buf is not None:
        #                 unit[global_param_index] = torch.empty_like(buf, device=self.rank, pin_memory=True)
        #                 global_param_index += 1
        #     self.cpu_cache_queue.append(unit)
        #################################################################################
        ...
        print(f"rank:{self.rank}, _special_layers_in_rank: {_special_layers_in_rank}")
        ...
        # print(f"rank:{self.rank}, {self.transformer_layer_info}")
        # print(f"rank:{self.rank}, {self.special_layers_info}")
        # exit(0)

        from param_swapper import AsyncParameterSwapper
        if self.transformer_layer_info is not None and self.special_layers_info is not None:
            self.param_swapper_handler = AsyncParameterSwapper(
                ds_config, 
                special_layer_id_in_rank=self.special_layers_in_rank,
                layer_count=self.max_len, 
                model_dtype=model_dtype, 
                rank=self.rank,
                transformer_layer_info=self.transformer_layer_info,
                special_layers_info=self.special_layers_info,
                transformer_layer=self.shared_model[1][0],
                special_layers_in_rank=_special_layers_in_rank
            )
        else:
            
            self.param_swapper_handler = AsyncParameterSwapper(ds_config,
                                                               self.special_layers_in_rank,
                                                               self.max_len, model_dtype,
                                                               self.rank,
                                                               transformer_layer_info=self.transformer_layer_info,
                                                               special_layers_info=self.special_layers_info,
                                                               transformer_layer=self.shared_model[1][0],
        ...
        self.shared_model_nvme_handler =  shared_optim_cpu.SharedModelNVMe_worker7(shared_model,
                                                                             self.param_swapper_handler,
                                                                             self.special_layers_in_rank,
                                                                             self.rank
                                                                             )
        dist.barrier()
        #################################################################################
        self.pcm.print("rank%d: shared_model_nvme_handler initialized" % self.rank)
        ...
        for vlayer_id, (optim, (_,X_names,Y_names), empty_vlayer) in enumerate(zip(shared_optimizer, shared_model, empty_model)):
            local_vlayer = local_model_gpu.LocalModelGPU_work7(optim.shared_model, empty_vlayer, self.shared_model_nvme_handler, self.cuda_cache_queue, special_layers_in_rank, vlayer_id, X_names, Y_names, self.rank, self.world_size, no_pin_model=args.no_pin_model, no_pin_grad_buf=args.no_pin_grad_buf) 
        ...
        self.update_handler = shared_optim_cpu.UpdateInBkgd_worker7(shared_optimizer, lr_scheduler, self.local_model, self.shared_model_nvme_handler, self.swapout_grad_handler, self.cuda_cache_queue, self.special_layers_in_rank, self.rank, nvprof=self.nvprof)
        # 
        ...
        syncpin_handler = shared_optim_cpu.SyncPinModelInBkgd_worker7(shared_optimizer, self.local_model, self.shared_model_nvme_handler, self.rank, nvprof=self.nvprof)

        self.prefetch_model_handler = local_model_gpu.PrefetchLocalModelGPU_worker7(syncpin_handler, self.local_model, self.rank, swapin_stream=None, compute_stream=self.default_stream, nvprof=self.nvprof)
    ...
    def _a_pack_forward_an_ubatch(self, vt, ubatch_idx, ubatch_size,
        ...
        if not vt.has_criterion: # not last pack yet
            ...
            if m.medium == "DAT": # Get one microbatch data
                ...
                p2p_wait_end = time.time()
                p2p_wait_duration = p2p_wait_end - p2p_wait_start
    ...
    def run_training_loop(self):
        ...
        self.p2p_wait_time_fwd = []
        self.p2p_wait_time_bwd = []
    ...
    def finish(self): 
        ...
        avg_p2p_wait_time_fwd = np.mean(self.p2p_wait_time_fwd[self.avg_it:]) if len(self.p2p_wait_time_fwd) > self.avg_it else 0.0
        avg_p2p_wait_time_bwd = np.mean(self.p2p_wait_time_bwd[self.avg_it:]) if len(self.p2p_wait_time_bwd) > self.avg_it else 0.0
        total_p2p_wait_time = avg_p2p_wait_time_fwd + avg_p2p_wait_time_bwd
        ...
        if self.rank == 0:
            ...
            if total_p2p_wait_time > 0:
                p2p_percentage = (total_p2p_wait_time / avg_iter_time) * 100
