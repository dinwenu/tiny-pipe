...
def cached_path(
    ...
    # 如果输入是远程 URL，则调用 get_from_cache 方法从缓存中获取文件（如果需要则下载）
...
# 获取指定 S3 对象的 ETag。ETag（Entity Tag）是 Amazon S3 用来标识对象的一个标识符
# url：S3 对象的 URL
@s3_request
def s3_etag(url, proxies=None):
    """Check ETag on S3 object."""
    # 使用 boto3.resource 创建一个 S3 资源对象
