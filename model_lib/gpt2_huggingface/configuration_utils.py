...
class PretrainedConfig(object):
    ...
    # 2.从一个字典和额外的关键字参数构建一个config类的实例并返回
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs) -> "PretrainedConfig":
        ...
        """
        # 加载JSON文件，返回配置字典 和 未使用的关键字参数
        config_dict, kwargs = cls.get_config_dict(pretrained_model_name_or_path, **kwargs)
        # 从一个字典和额外的关键字参数构建一个config类的实例并返回
        return cls.from_dict(config_dict, **kwargs)

    # 加载JSON文件，返回配置字典和未使用的关键字参数
    @classmethod
    def get_config_dict(
        ...
        # 检查 pretrained_model_name_or_path 是否是一个目录路径：
        # 如果是一个目录路径，则假定该目录包含一个配置文件，使用 os.path.join 将目录路径和预定义的配置文件名 CONFIG_NAME 连接起来
        elif os.path.isdir(pretrained_model_name_or_path):
            config_file = os.path.join(pretrained_model_name_or_path, CONFIG_NAME)
        # 检查 pretrained_model_name_or_path 是否是一个文件路径或远程 URL：
        # 如果是一个文件路径或远程 URL，则直接使用该路径或 URL 作为配置文件路径
        elif os.path.isfile(pretrained_model_name_or_path) or is_remote_url(pretrained_model_name_or_path):
            config_file = pretrained_model_name_or_path
        # 如果 pretrained_model_name_or_path 既不是快捷名称、目录路径，也不是文件路径或 URL，则假定它是一个模型名称，
        # 并使用 hf_bucket_url 函数构建一个 Hugging Face 存储库的 URL
        else:
    ...
    def from_dict(cls, config_dict: Dict, **kwargs) -> "PretrainedConfig":
        ...
        Returns:
            :class:`PretrainedConfig`: An instance of a configuration object
        """
        # 使用 pop 方法获取 return_unused_kwargs 参数，如果不存在则默认为 False
        return_unused_kwargs = kwargs.pop("return_unused_kwargs", False)

        # 使用 config_dict 中的参数实例化配置对象
        ...
        to_remove = []
        # 如果配置对象有对应的属性，则使用 setattr 更新该属性，并将键添加到 to_remove 列表中
        for key, value in kwargs.items():
            if hasattr(config, key):
                setattr(config, key, value)
                to_remove.append(key)
        # 遍历 to_remove 列表，从 kwargs 中移除已使用的键
