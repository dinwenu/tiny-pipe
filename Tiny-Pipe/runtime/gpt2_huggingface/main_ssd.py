...
def create_optimizer_new(args, model):
    optimizer = []
    ...
    for vlayer, _, _ in model:
        if len(list(vlayer.parameters())) == 0:
            ...
            param_optimizer = list(vlayer.named_parameters())
            optim = DeepSpeedCPUAdam(param_optimizer, 
                                    lr=args.learning_rate, 
                                    eps=args.adam_epsilon) 
            optimizer.append(optim)
    return optimizer
...
if __name__ == "__main__":    
    ...
    import runtime_ssd
    runtime_ssd.run(args, real_dataset, create_model, create_optimizer, get_lr_sched=get_lr_sched, compute_loss=compute_loss, save_model=save_model)
